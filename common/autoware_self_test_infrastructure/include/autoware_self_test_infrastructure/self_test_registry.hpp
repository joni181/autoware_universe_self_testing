...
public:
  ...
  void unregister_tester(const std::string & module_id);

  std::vector<TesterHandle> list_testers() const;

  std::optional<TesterHandle> get_tester(const std::string & module_id) const;
  ...
  std::shared_ptr<ITesterComponent> try_get_tester(const std::string & module_id) const;
