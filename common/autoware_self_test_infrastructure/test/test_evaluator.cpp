...
types::TestRunResult make_run_result(std::initializer_list<types::TestResultStatus> results)
{
...
{
  infra::Evaluator evaluator;
  const auto run_result = make_run_result(
    {types::TestResultStatus::PASS, types::TestResultStatus::FAIL, types::TestResultStatus::PASS});
...
TEST(evaluator, returns_true_when_all_tests_passed)
{
  infra::Evaluator evaluator;
  const auto run_result = make_run_result(
    {types::TestResultStatus::PASS, types::TestResultStatus::PASS, types::TestResultStatus::PASS});
  const auto eval = evaluator.evaluate(run_result);
...
TEST(evaluator, treats_skipped_tests_as_non_failures)
{
  infra::Evaluator evaluator;
  const auto run_result = make_run_result(
    {types::TestResultStatus::PASS, types::TestResultStatus::SKIP, types::TestResultStatus::PASS});
  const auto eval = evaluator.evaluate(run_result);
